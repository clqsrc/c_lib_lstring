...
#ifndef _L_STRING_BASE_C_
#define	_L_STRING_BASE_C_

//#include "lstring.h"
#include "lstring_base.h"
#include "lstring_app.c"
...
void init_mempool(mempool * pmem)
{
    //mempool * pmem;
    ...
    //2022 v2.0 释放标志  //64 位下是 1717986918，这是 pc 字节序，其他机器还不一定，所以不能用它来和 'ffff' 硬比较
    //同理 'cccc' 在 pc 64 位下是 1667457891
    pmem->free_check_tag_int = _lstring_free_check_tag_int_ALLOC_;
    //pmem->free_check_tag_char[0] = 'c';
    //pmem->free_check_tag_char[1] = 'c';
    //pmem->free_check_tag_char[2] = 'c';
    //pmem->free_check_tag_char[3] = 'c';

    //printf("Pool_Free: %d", pmem->free_check_tag_int);

    //return pmem;
}

//现在尽量不要用这个函数，因为它没有内存释放检测
mempool makemem()
{
	mempool mem; 
...
void Pool_Free(struct MemPool * pool)
{
    ...
    //2022 v2.0 释放标志  //64 位下是 1717986918，这是 pc 字节序，其他机器还不一定，所以不能用它来和 'ffff' 硬比较
    pool->free_check_tag_int = _lstring_free_check_tag_int_FREE_;
    //pool->free_check_tag_char[0] = 'f';
    //pool->free_check_tag_char[1] = 'f';
    //pool->free_check_tag_char[2] = 'f';
    //pool->free_check_tag_char[3] = 'f';

    //printf("Pool_Free: %d", pool->free_check_tag_int);
...
//lstring * NewString(const char * str, struct MemPool * pool)
{
    _MM_CheckFree_LStringMemPool_(pool);
...
void LString_Append(lstring * _s, lstring * _add)
{
	...
	tmp = (char *)malloc(s.len + add.len + 1); //还要留最后 \0 的位置 
...
{
    printf_err1("CheckFree_LString \r\n");

    CheckFree_LStringMemPool(s->pool);
