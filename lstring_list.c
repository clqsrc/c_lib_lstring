...
{
	lstring_list * list = malloc(sizeof(struct LStringList));
...
void LStringList_Add(lstring_list * list, lstring * s)
{
	...
	if (list->_buf_count < list->count)  //空间不足了就要扩大
	{
		...
		list->_list_addr_buf = malloc(list->_buf_count * sizeof(void *));
		list->items = list->_list_addr_buf;
	}
...
void BindStringListFunctions(lstring_list * list) 
{
	...
	list->AddConst = LStringList_Add;
    list->AddCString = LStringList_Add;
